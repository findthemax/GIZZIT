...
const InputDropdown = ({data, dispatch, errors, ...props}) => {
    ...
    const {param, label, value, showState, options} = data
        ...
        <Fragment>
            <div className={errorArray.length > 0 ? "input-dropdown it-error" : "input-dropdown"}>
                ...
                <ul className="dropdown-list" style={showState ? { display: "flex" } : { display: "none" }}>
                    {options.map(option => option.available && <li key={option.value} onClick={() => dispatch({type: "setValue", value: option.value, data, option})}>{option.label}</li>)}
                </ul>
